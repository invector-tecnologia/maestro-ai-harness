...
- **Active (llm-driven) engine** — when a provider model is online, Maestro leads the conversation live, one focused question at a time, and never imposes creating a persona or file. As it learns, it periodically **summarizes what it has gathered and asks whether that is enough to proceed to handoff**. If you say it is enough, it proposes the governed file changes (scopes plus the persona/skill additions they imply) as a structured set and prepares the handoff; if you say it is not enough, it offers a short list of next-step options and keeps gathering, checking again periodically. Proposals are **staged for confirmation**, never written silently. Approve with `y` to write every change through markdown governance; choose `n` to keep talking and let Maestro refine. The immutable Maestro persona is always filtered out of any proposal.
